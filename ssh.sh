...
function check_traffic(){
	...
	target_ip="168.63.129.16"
        ...
        if [ `grep -c "ack" /home/$username/test_tcpdump.txt` -eq '0' ]; then
        ...
        if [ $os_flag == "r" ]; then
                ...
                firewall_status=$(systemctl status firewalld.service | sed -n '3p' | awk '{print $3}')
                        ...
                        if [[ "$firewall_status" =~ "running" ]]; then
                                ...
                                firewall_port_status=$(iptables -L | grep $ssh_running_port)
                                firewall_ip_status=$(iptables -L | grep $client_ip)
                                if [ -z $firewall_port_status ]; then
                                        ...
                                        echo "++++    Port allowed checking --- Failed --- Please make sure Port $ssh_running_port is allowed by Firewall!" >> $current_dir/suggestion.txt

                                fi

                                if [ -z $firewall_ip_status ]; then
                                        ...
                                        echo "++++    Client IP allowed checking --- Failed --- Please make sure Client IP: $client_ip is allowed by Firewall!" >> $current_dir/report.txt
			...
			check_customized_sshport

        else

                firewall_status=$(systemctl status ufw.service | sed -n '3p' | awk '{print $3}')

                        if [[ "$firewall_status" =~ "exited" ]]; then
...
function check_ssh_status(){
        ...
        ssh_status=$(systemctl status sshd.service | sed -n '3p' | awk '{print $3}' )
                ...
                if [[ "$ssh_status" =~ "running" ]]; then
                        ...
                        echo "++++    SSH running status checking --- Failed --- Please make sure SSH service is running!" >> $current_dir/suggestion.txt
...
function check_ssh_config(){
        ...
        ssh_port_check=$(cat /etc/ssh/sshd_config | grep Port | sed -e '/#/d')
	...
	ssh_running_port=$(netstat -tulpn | grep ssh | sed -e '/tcp6/d' | awk '{print $4}' |  awk -F':' '{print $2}')
        ...
        if [ -z "$PasswordAuthentication" ] || [[ "$PasswordAuthentication" =~ "yes" ]]; then
                ...
                echo "++++    PasswordAuthentication enabled checking --- Failed --- Please make sure PasswordAuthentication is enabled!" >> $current_dir/suggestion.txt
        fi
        ...
        if [ -z "$PubkeyAuthentication" ] || [[ "$PubkeyAuthentication" =~ "yes" ]]; then
                ...
                echo "++++    PubkeyAuthentication enabled checking --- Failed --- Please make sure PubkeyAuthentication is enabled!" >> $current_dir/suggestion.txt

        fi

}

function check_customized_sshport(){
	...
	if [ "$ssh_running_port" != "22" ]; then
		...
		custom_port=$(firewall-cmd --list-ports | grep $ssh_running_port | sed -e '/#/d')
...
function check_user_status(){
	...
	if [ "$deny_allow" == d ]; then
                ...
                echo "++++    Username allowed checking --- Failed --- Please make sure Username $username is allowed by SSH1!" >> $current_dir/suggestion.txt
	else
...
function check_hostdeny(){
        ...
        if [ -n "$hostdeny_status" ]; then
                ...
                echo "++++    Clinet IP allowed checking --- Failed --- Please make sure Client IP: $client_ip is allowed by this VM(host level)!" >> $current_dir/suggestion.txt
...
function suggestion(){
        ...
        if [ ! -f "$current_dir/suggestion.txt" ];then
                ...
                echo "++++    No major issue were found yet" >> $current_dir/suggestion.txt
